...
int main(int argc, char **argv)
{
    ...
    string s = (argc==2) ? string(argv[1]) : string(DATA_PATH) + "/3holes.obj";
    DrawableTrimesh<> m_in(s.c_str());

    // make tetmesh
    vector<uint>   edges, tets;
    vector<float> verts;
    float vol_thresh = 0.01 * m_in.bbox().diag(); // force tets to be smaller than 5% of bbox diag
    char opt[100];
    ...
    string pov("0.217281 -0.976103 0.00348926 0 0.94495 0.209448 -0.2514 0 0.244662 0.0579212 0.967876 0 -0.747865 0.366815 -2.40522 1 1.32704 2.47434");
